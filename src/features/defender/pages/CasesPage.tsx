...
export const DefenderCasesPage = () => {
  ...
  const filteredReports = reports.filter(r => {
    ...
    if (filter === 'closed') return r.status === 'RESOLVED'
    return true
  })
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      IN_FOLLOW_UP: 'bg-orange-100 text-orange-800',
  ...
  return (
    <div className="w-full px-8 py-8 pb-16">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Mis Casos Legales</h1>
        <p className="text-gray-600">Seguimiento y gestión de procesos legales</p>
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-6 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-600">
                        Registrado: {new Date(report.createdAt || '').toLocaleDateString('es-ES')}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-3 py-1 rounded text-xs font-bold ${getPriorityColor(report.priority)}`}>
                        {report.priority === 'HIGH' ? '🔴 ALTA PRIORIDAD' : report.priority === 'MEDIUM' ? '🟡 MEDIA' : '🔵 BAJA'}
                      </span>
                    </div>
                  ...
                  {report.status === 'IN_FOLLOW_UP' && (
                    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center gap-2">
                      <CheckCircle className="w-5 h-5 text-blue-600" />
                      <span className="text-sm text-blue-800">
                        <strong>Nota:</strong> Caso en seguimiento legal. Próxima audiencia por definir.
                      </span>
                    </div>
                  )}
                </div>
